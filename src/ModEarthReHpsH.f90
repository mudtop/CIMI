Module ModCimiPlanet
  ...
  character(len=2) :: NameSpeciesExtension_I(nspec)=(/'_h','ps','_e'/)

  logical :: UseRecirculatedPs = .true.

  !named index for species order
  integer :: H_=1, PsHp_=2, e_=3
  ...
  real,parameter,dimension(nspec-1) :: a0_I = (/-18.767,-18.767/)
  real,parameter,dimension(nspec-1) :: a1_I = (/-0.11017,-0.11017/)
  real,parameter,dimension(nspec-1) :: a2_I = (/-3.8173e-2,-3.8173e-2/)
  real,parameter,dimension(nspec-1) :: a3_I = (/-0.1232,-0.1232/)
  real,parameter,dimension(nspec-1) :: a4_I = (/-5.0488e-2,-5.0488e-2/)

  !set species Mass in amu
  real,parameter,dimension(nspec)   :: amu_I = (/1.0,1.0,5.4462e-4/)


  !set density and temp factor
  real, dimension(nspec) :: dFactor_I =(/1.0,0.0,1.0/)
  real, dimension(nspec) :: tFactor_I =(/1.0,0.0,0.128205/)

  !set plot parameters
  character(len=300), parameter :: &
       NamePlotVar='x y P[nP] HpP[nP] PsHpP[nP] eP[nP] ' &
       //'Ppar[nP] HpPpar[nP] PsHpPpar[nP] ePpar[nP] ' &
       //'Phot[nP] HpPhot[nP] PsHpPhot[nP] ePhot[nP] '&
       //'Pparhot[nP] HpPparhot[nP] PsHpPparhot[nP] ePparhot[nP] ' &
       // 'N[/m3] HpN[/m3] PsHpN[/m3] eN[/m3] Beq[T] Vol[m3/Wb] Pot[Volts] FAC[Amp/m2] ' & 
  ...
  character(len=*), parameter :: &
       NameVarCouple = 'Rho Ppar p HpRho HpPpar HpP PsHpRho PsHpP Pe PePar'
  ...
  character(len=500), parameter :: &
       ...
       'RbSumPsHp RcSumPsHp PsHpDrift PsHpBfield PsHpChargeEx PsHpWaves '// &
       'PsHpStrongDiff PsHpDecay PsHpLossCone PsHpDriftIn PsHpDriftOut '// &
